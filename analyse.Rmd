...
# Kde je nejvíc kandidátů na mandát?
...
library(readxl)
zastupitelstva <- read_excel("kvrzcoco.xlsx")
library(dplyr)
zastupitelstva_mandaty <- zastupitelstva %>%
  select(KODZASTUP, MANDATY, NAZEVOBCE)
kandidati <- read_excel("kvrk.xlsx")
kandidati_obce <- kandidati %>%
  group_by(KODZASTUP) %>%
